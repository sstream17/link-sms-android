...
import xyz.stream.messenger.api.entity.LoginResponse
import xyz.stream.messenger.api.implementation.AccountEncryptionCreator
...
class ActivateWearActivity : Activity() {
    ...
    private fun queryEndpoint() {
        ...
        handler.postDelayed({
            Thread {
                ...
                val response: xyz.stream.messenger.api.entity.LoginResponse? = try {
    ...
    private fun activated(response: xyz.stream.messenger.api.entity.LoginResponse) {
    ...
    private fun checkPassword(response: xyz.stream.messenger.api.entity.LoginResponse, password: String?) {
        ...
        Thread {
            val encryptionCreator = xyz.stream.messenger.api.implementation.AccountEncryptionCreator(this@ActivateWearActivity, password)
            ...
            try {
                ...
                setResult(xyz.stream.messenger.api.implementation.LoginActivity.RESULT_START_NETWORK_SYNC)
                finish()
            } catch (e: Exception) {
                runOnUiThread {
                    Toast.makeText(this@ActivateWearActivity, xyz.stream.messenger.api.implementation.R.string.api_wrong_password,
