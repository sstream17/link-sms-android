...
import com.stream_suite.link.shared.util.isDarkColor

class RateItDialog : FloatingTutorialActivity(), TutorialFinishedListener {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        com.stream_suite.link.api.implementation.firebase.AnalyticsHelper.rateItPromptShown(this)
    }

    override fun onTutorialFinished() {
        try {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=$packageName")))
            com.stream_suite.link.api.implementation.firebase.AnalyticsHelper.rateItClicked(this)
