...
import com.stream_suite.link.shared.service.NewMessagesCheckService
...
object DataSource {
    ...
    var _encryptor: com.stream_suite.link.encryption.EncryptionUtils? = null
    ...
    @Synchronized
    private fun encryptor(context: Context): com.stream_suite.link.encryption.EncryptionUtils? {
        if (_encryptor == null) {
    ...
    fun getMessageDetails(context: Context, messageId: Long): Spanned {
        try {
            ...
            if (message.mimeType != MimeType.TEXT_PLAIN) {
                val bytes = com.stream_suite.link.api.implementation.BinaryUtils.getMediaBytes(context, message.data, message.mimeType, false)
