...
import xyz.stream.messenger.api.implementation.firebase.FirebaseUploadCallback
...
open class ApiUploadService : Service() {
    ...
    private var encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils? = null
    ...
    private fun uploadData() {
        ...
        if (encryptionUtils == null) {
            val intent = Intent(this, xyz.stream.messenger.api.implementation.LoginActivity::class.java)
    ...
    private fun uploadMessages() {
        ...
        if (cursor.moveToFirst()) {
            val messages = ArrayList<xyz.stream.messenger.api.entity.MessageBody>()
            ...
            do {
                ...
                val message = xyz.stream.messenger.api.entity.MessageBody(m.id, m.conversationId, m.type, m.data,
            ...
            for (page in pages) {
                val request = xyz.stream.messenger.api.entity.AddMessagesRequest(Account.accountId, page.toTypedArray())
                try {
    ...
    private fun uploadConversations() {
        ...
        if (cursor.moveToFirst()) {
            val conversations = arrayOfNulls<xyz.stream.messenger.api.entity.ConversationBody>(cursor.count)

            do {
                val c = Conversation()
                c.fillFromCursor(cursor)
                c.encrypt(encryptionUtils!!)
                val conversation = xyz.stream.messenger.api.entity.ConversationBody(c.id, c.colors.color,
            ...
            val request = xyz.stream.messenger.api.entity.AddConversationRequest(Account.accountId, conversations)
            var result: Response<*>?
    ...
    private fun uploadBlacklists() {
        ...
        if (cursor.moveToFirst()) {
            val blacklists = arrayOfNulls<xyz.stream.messenger.api.entity.BlacklistBody>(cursor.count)

            do {
                val b = Blacklist()
                b.fillFromCursor(cursor)
                b.encrypt(encryptionUtils!!)
                val blacklist = xyz.stream.messenger.api.entity.BlacklistBody(b.id, b.phoneNumber, b.phrase)

                blacklists[cursor.position] = blacklist
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddBlacklistRequest(Account.accountId, blacklists)
            val result = try {
    ...
    private fun uploadScheduledMessages() {
        ...
        if (cursor.moveToFirst()) {
            val messages = arrayOfNulls<xyz.stream.messenger.api.entity.ScheduledMessageBody>(cursor.count)

            do {
                val m = ScheduledMessage()
                m.fillFromCursor(cursor)
                m.encrypt(encryptionUtils!!)
                val message = xyz.stream.messenger.api.entity.ScheduledMessageBody(m.id, m.to, m.data,
                        m.mimeType, m.timestamp, m.title, m.repeat)

                messages[cursor.position] = message
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddScheduledMessageRequest(Account.accountId, messages)
            val result = try {
    ...
    private fun uploadDrafts() {
        ...
        if (cursor.moveToFirst()) {
            val drafts = arrayOfNulls<xyz.stream.messenger.api.entity.DraftBody>(cursor.count)

            do {
                val d = Draft()
                d.fillFromCursor(cursor)
                d.encrypt(encryptionUtils!!)
                val draft = xyz.stream.messenger.api.entity.DraftBody(d.id, d.conversationId, d.data, d.mimeType)

                drafts[cursor.position] = draft
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddDraftRequest(Account.accountId, drafts)
            val result = try {
    ...
    private fun uploadTemplates() {
        ...
        if (cursor.moveToFirst()) {
            val templates = arrayOfNulls<xyz.stream.messenger.api.entity.TemplateBody>(cursor.count)

            do {
                val t = Template()
                t.fillFromCursor(cursor)
                t.encrypt(encryptionUtils!!)
                val template = xyz.stream.messenger.api.entity.TemplateBody(t.id, t.text)

                templates[cursor.position] = template
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddTemplateRequest(Account.accountId, templates)
            val result = try {
    ...
    private fun uploadFolders() {
        ...
        if (cursor.moveToFirst()) {
            val folders = arrayOfNulls<xyz.stream.messenger.api.entity.FolderBody>(cursor.count)

            do {
                val f = Folder()
                f.fillFromCursor(cursor)
                f.encrypt(encryptionUtils!!)
                val folder = xyz.stream.messenger.api.entity.FolderBody(f.id, f.name, f.colors.color, f.colors.colorDark, f.colors.colorLight, f.colors.colorAccent)

                folders[cursor.position] = folder
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddFolderRequest(Account.accountId, folders)
            val result = try {
    ...
    private fun uploadAutoReplies() {
        ...
        if (cursor.moveToFirst()) {
            val replies = arrayOfNulls<xyz.stream.messenger.api.entity.AutoReplyBody>(cursor.count)

            do {
                val r = AutoReply()
                r.fillFromCursor(cursor)
                r.encrypt(encryptionUtils!!)
                val reply = xyz.stream.messenger.api.entity.AutoReplyBody(r.id, r.type, r.pattern, r.response)

                replies[cursor.position] = reply
            } while (cursor.moveToNext())

            val request = xyz.stream.messenger.api.entity.AddAutoReplyRequest(Account.accountId, replies)
            val result = try {
    ...
    private fun processMediaUpload(manager: NotificationManagerCompat,
        ...
        if (media.moveToFirst()) {
            ...
            do {
                ...
                val bytes = xyz.stream.messenger.api.implementation.BinaryUtils.getMediaBytes(this, message.data, message.mimeType, true)

                ApiUtils.uploadBytesToFirebase(Account.accountId, bytes, message.id, encryptionUtils, xyz.stream.messenger.api.implementation.firebase.FirebaseUploadCallback {
    ...
    companion object {
        ...
        public fun uploadContacts(context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils) {
            val cursor = DataSource.getContacts(context)

            if (cursor.moveToFirst()) {
                val contacts = ArrayList<xyz.stream.messenger.api.entity.ContactBody>()
                ...
                do {
                    ...
                    val contact = if (c.type != null) {
                        xyz.stream.messenger.api.entity.ContactBody(c.id, c.phoneNumber, c.idMatcher, c.name, c.type!!, c.colors.color, c.colors.colorDark, c.colors.colorLight, c.colors.colorAccent)
                    } else {
                        xyz.stream.messenger.api.entity.ContactBody(c.id, c.phoneNumber, c.idMatcher, c.name, c.colors.color, c.colors.colorDark, c.colors.colorLight, c.colors.colorAccent)
                    }
        ...
        fun uploadContacts(contacts: List<xyz.stream.messenger.api.entity.ContactBody>) {
            val startTime = TimeUtils.now
            ...
            for (page in pages) {
                val request = xyz.stream.messenger.api.entity.AddContactRequest(Account.accountId, page.toTypedArray())
                try {
