...
object DataSource {
    ...
    var _encryptor: xyz.stream.messenger.encryption.EncryptionUtils? = null
    ...
    @Synchronized
    private fun encryptor(context: Context): xyz.stream.messenger.encryption.EncryptionUtils? {
        if (_encryptor == null) {
    ...
    fun getMessageDetails(context: Context, messageId: Long): Spanned {
        try {
            ...
            if (message.mimeType != MimeType.TEXT_PLAIN) {
                val bytes = xyz.stream.messenger.api.implementation.BinaryUtils.getMediaBytes(context, message.data, message.mimeType, false)
...
//                    message.mimeType, false, useApi)
     ...
     */
    @JvmOverloads fun insertDraft(context: Context?, conversationId: Long, data: String, mimeType: String, useApi: Boolean = true, useSnippetApi: Boolean = useApi, scheduledMessage: ScheduledMessage = ScheduledMessage()): Long {
        if (context == null) {
            return -1L
        }
