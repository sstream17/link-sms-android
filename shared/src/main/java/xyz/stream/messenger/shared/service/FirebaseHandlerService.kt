...
import xyz.stream.messenger.api.implementation.firebase.FirebaseDownloadCallback
import xyz.stream.messenger.api.implementation.firebase.FirebaseUploadCallback
import xyz.stream.messenger.api.implementation.firebase.MessengerFirebaseMessagingService
...
import xyz.stream.messenger.shared.service.jobs.SignoutJob
import xyz.stream.messenger.shared.service.jobs.SubscriptionExpirationCheckJob
import xyz.stream.messenger.shared.service.notification.Notifier
...
class FirebaseHandlerService : IntentService("FirebaseHandlerService") {
    ...
    override fun onHandleIntent(intent: Intent?) {
        if (intent != null && intent.action != null && intent.action == xyz.stream.messenger.api.implementation.firebase.MessengerFirebaseMessagingService.ACTION_FIREBASE_MESSAGE_RECEIVED) {
    ...
    companion object {
        ...
        @Throws(JSONException::class)
        private fun addMessage(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
        ...
        private fun addMessageAfterFirebaseDownload(context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils, message: Message, to: String? = null) {
            ...
            val callback = xyz.stream.messenger.api.implementation.firebase.FirebaseDownloadCallback {
                ...
                if (to != null) {
                    val bytes = xyz.stream.messenger.api.implementation.BinaryUtils.getMediaBytes(context, message.data, message.mimeType, true)
                    ApiUtils.uploadBytesToFirebase(Account.accountId, bytes, message.id, encryptionUtils, xyz.stream.messenger.api.implementation.firebase.FirebaseUploadCallback { }, 0)
                }
            }
        ...
        @Throws(JSONException::class)
        private fun addConversation(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
        ...
        @Throws(JSONException::class)
        private fun updateContact(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            try {
                val contact = Contact()
        ...
        @Throws(JSONException::class)
        private fun addContact(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
        ...
        @Throws(JSONException::class)
        private fun updateConversation(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            try {
                val conversation = Conversation()
                conversation.id = getLong(json, "id")
                conversation.title = encryptionUtils!!.decrypt(json.getString("title"))
        ...
        @Throws(JSONException::class)
        private fun updateConversationTitle(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            try {
        ...
        @Throws(JSONException::class)
        private fun updateConversationSnippet(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            try {
        ...
        @Throws(JSONException::class)
        private fun addDraft(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val draft = Draft()
        ...
        @Throws(JSONException::class)
        private fun addBlacklist(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val id = getLong(json, "id")
        ...
        @Throws(JSONException::class)
        private fun addScheduledMessage(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
        ...
        @Throws(JSONException::class)
        private fun updatedScheduledMessage(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val message = ScheduledMessage()
            message.id = getLong(json, "id")
            message.to = encryptionUtils!!.decrypt(json.getString("to"))
            message.data = encryptionUtils.decrypt(json.getString("data"))
        ...
        @Throws(JSONException::class)
        private fun addTemplate(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
        ...
        @Throws(JSONException::class)
        private fun updateTemplate(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val template = Template()
            template.id = getLong(json, "device_id")
            template.text = encryptionUtils!!.decrypt(json.getString("text"))
        ...
        @Throws(JSONException::class)
        private fun addAutoReply(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val reply = AutoReply()
        ...
        @Throws(JSONException::class)
        private fun updateAutoReply(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val reply = AutoReply()
            reply.id = getLong(json, "device_id")
            reply.type = json.getString("type")
            reply.pattern = encryptionUtils!!.decrypt(json.getString("pattern"))
        ...
        @Throws(JSONException::class)
        private fun addFolder(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val folder = Folder()
        ...
        @Throws(JSONException::class)
        private fun updateFolder(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            val folder = Folder()
            folder.id = getLong(json, "device_id")
            folder.name = encryptionUtils!!.decrypt(json.getString("name"))
            folder.colors.color = json.getInt("color")
        ...
        @Throws(JSONException::class)
        private fun forwardToPhone(json: JSONObject, context: Context, encryptionUtils: xyz.stream.messenger.encryption.EncryptionUtils?) {
            if (!Account.primary) {
                return
            }
