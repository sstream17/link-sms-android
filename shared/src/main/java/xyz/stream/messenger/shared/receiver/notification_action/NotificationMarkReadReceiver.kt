...
open class NotificationMarkReadReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        xyz.stream.messenger.shared.receiver.notification_action.NotificationMarkReadReceiver.Companion.handle(intent, context)
    }
    ...
    companion object {
        ...
        fun handle(intent: Intent?, context: Context) {
            Thread {
                val conversationId = intent?.getLongExtra(xyz.stream.messenger.shared.receiver.notification_action.NotificationMarkReadReceiver.Companion.EXTRA_CONVERSATION_ID, -1) ?: return@Thread
