...
import xyz.stream.messenger.shared.util.isDarkColor

class RateItDialog : FloatingTutorialActivity(), TutorialFinishedListener {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        xyz.stream.messenger.api.implementation.firebase.AnalyticsHelper.rateItPromptShown(this)
    }

    override fun onTutorialFinished() {
        try {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=$packageName")))
            xyz.stream.messenger.api.implementation.firebase.AnalyticsHelper.rateItClicked(this)
