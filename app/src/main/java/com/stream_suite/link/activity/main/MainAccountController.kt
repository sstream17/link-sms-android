...
import com.stream_suite.link.activity.InitialLoadActivity
import com.stream_suite.link.activity.MessengerActivity
import com.stream_suite.link.activity.OnboardingActivity
...
import com.stream_suite.link.shared.service.FirebaseTokenUpdateCheckService
...
class MainAccountController(private val activity: MessengerActivity) {
    ...
    fun startLoad(requestCode: Int) {
        if (requestCode == MessengerActivityExtras.REQUEST_ONBOARDING) {
            val hasTelephone = activity.packageManager.hasSystemFeature(PackageManager.FEATURE_TELEPHONY)
            val login = Intent(activity, InitialLoadActivity::class.java)
            login.putExtra(com.stream_suite.link.api.implementation.LoginActivity.ARG_SKIP_LOGIN, hasTelephone)
