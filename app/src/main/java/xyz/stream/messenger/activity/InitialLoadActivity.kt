...
import xyz.stream.messenger.api.implementation.ActivateActivity
...
import xyz.stream.messenger.shared.data.model.Contact
import xyz.stream.messenger.shared.service.ApiDownloadService
import xyz.stream.messenger.shared.service.ApiUploadService
import xyz.stream.messenger.shared.util.*
import xyz.stream.messenger.shared.util.listener.ProgressUpdateListener
...
open class InitialLoadActivity : AppCompatActivity(), ProgressUpdateListener {
    ...
    private fun requestPermissions() {
        ...
        if (!resources.getBoolean(R.bool.is_tablet) && defaultApp != null && defaultApp.isNotBlank() &&
            ...
            // Link is not currently the default
            ...
            if (promptedForDefaultSMS && !AndroidVersionUtil.isAndroidQ) {
                ...
                // Google must know, if there are no permissions granted and the user allows Link to be
    ...
    public override fun onActivityResult(requestCode: Int, responseCode: Int, data: Intent?) {
        if (requestCode == PermissionsUtils.REQUEST_DEFAULT_SMS_APP) {
            ...
            when (responseCode) {
                ...
                xyz.stream.messenger.api.implementation.LoginActivity.RESULT_START_DEVICE_SYNC -> {
                    startDatabaseSync()
                    startUploadAfterSync = true
                }
                xyz.stream.messenger.api.implementation.LoginActivity.RESULT_START_NETWORK_SYNC -> {
                ...
                xyz.stream.messenger.api.implementation.ActivateActivity.RESULT_FAILED -> finish()
            }
        }
    }
    ...
    private fun startLogin() {
        ...
        val login = Intent(this, xyz.stream.messenger.api.implementation.LoginActivity::class.java)
    ...
    private fun startDatabaseSync() {
        Thread {
            ...
            xyz.stream.messenger.api.implementation.firebase.AnalyticsHelper.importFinished(this, importTime)
